...
namespace jettnet.mirage.extensions
{
    public static class CollectionExtensions
    {
        ...
        public static void WriteBytesAndSize(this JettWriter writer, byte[] buffer, int offset, int count)
        {
            // null is supported
        ...
        /// </summary>
        /// <param name="buffer">array or null</param>
        public static void WriteBytesAndSize(this JettWriter writer, byte[] buffer)
        {
            // buffer might be null, so we can't use .Length in that case
            writer.WriteBytesAndSize(buffer, 0, buffer != null ? buffer.Length : 0);
        }

        public static void WriteBytesAndSizeSegment(this JettWriter writer, ArraySegment<byte> buffer)
        {
            writer.WriteBytesAndSize(buffer.Array, buffer.Offset, buffer.Count);
        }
        
        public static void WriteList<T>(this JettWriter writer, List<T> list)
        {
            if (list is null)
            {
        ...
        public static void WriteArray<T>(this JettWriter writer, T[] array)
        {
            if (array is null)
            {
                writer.WritePackedInt32(-1);
        ...
        public static byte[] ReadBytesAndSize(this JettReader reader)
        {
        ...
        public static ArraySegment<byte> ReadBytesAndSizeSegment(this JettReader reader)
        {
            // count = 0 means the array was null
            // otherwise count - 1 is the length of the array
            uint count = reader.ReadPackedUInt32();
        ...
        public static byte[] ReadBytes(this JettReader reader, int count)
        {
        ...
        public static List<T> ReadList<T>(this JettReader reader)
        {
        ...
        public static T[] ReadArray<T>(this JettReader reader)
        {
            ...
            if (!reader.CanReadBits(length))
                throw new
                    EndOfStreamException($"Can't read {length} elements because it would read past the end of the stream.");
